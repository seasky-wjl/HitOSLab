...
long volatile jiffies=0;  // 记录从开机到的那个前时间的时钟中断发生次数（溢出了怎么办？）
long startup_time=0;
...
// 系统无事可作的时候，进程0会始终循环调用sys_pause(),以激活调度算法
// 此时它的状态可以是等待态，等待有其它可运行的进程；也可以叫运行态，因为它是唯一一个在CPU上运行的将进程，只不过运行的效果是等待
// 这里采用第二种方式，因为如果用第一种方式，那么/var/process.log会多处来许多进程0的状态切换而冗余
// 因此，打印的时候需要判断的那个前任务是否为0.如果是则不进行打印
int sys_pause(void)
{
